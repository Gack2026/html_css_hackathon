document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.btn-pricing').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            const course = btn.closest('.price-column').querySelector('h2').textContent;
            document.getElementById('course-select').value = selectedItem;
            document.getElementById('apply-form').querySelector('h2').textContent = `【${course}】に申し込む`;
        });
    });
    ...
    document.querySelector('.contact-form form').addEventListener('submit', e => {
        ...
        if (!errorFlag) {
            document.querySelector('.contact-form').innerHTML = '<h4>お問い合わせありがとうございます。</h4>';
        }
    });
